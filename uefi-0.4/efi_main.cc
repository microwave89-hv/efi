 ...
 * Le nom d'un disque.
 */
static const char * nom_disk( EFI_HANDLE h )
{
	static char txt[ 30 ] ;
	sprintf( txt , "EFI_HANDLE(%lX)" , h ) ;
	return txt ;
}
...
static EFI_STATUS FindPartitionableDevices( void )
{
    ...
    */
    ...
    for (i = 0; i < HandleCount; i++) {
        ...
        /*
         According to Windows GPT FAQ at www.microsoft.com/hwdev/tech/storage/GPT_FAQ.ASP,
         Microsoft tools will never partition removable media, whether it be floppy or
         superfloppy.
        */
        ...
        */
		...
		status = UEFI_call( ST-> BootServices-> HandleProtocol ,
			HandlePointer[ i ] , & BlockIoProtocol , & BlkIo ) ;
		cout << " " << max_table + 1 << ": Disk " << nom_disk( HandlePointer[ i ] ) ;
		cout << ": " << BlkIo-> Media-> LastBlock + 1 << " sectors, " << BytesToIeee(
			BlkIo-> Media-> LastBlock + 1 , BlkIo-> Media-> BlockSize ) << "\n    " ;
		cout.flush() ;
		// écrit caractère par caractère pour convertir chaque CHAR16 en char
		p = str ;
		while ( * p ) {
			char c = * p ;
			fputc( c , stdout ) ;
...
efi_main( EFI_HANDLE ImageHandle , EFI_SYSTEM_TABLE * SystemTable )
{
	...
	status = UEFI_call( SystemTable-> BootServices-> HandleProtocol ,
		ImageHandle , &my_LoadedImageProtocol , (void **) & loaded_image ) ;
	if ( EFI_ERROR( status ) ) {
