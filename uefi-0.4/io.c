...
enum type_fd
{
	...
	fd_console , /* console stdin, stdout, etc... */
	fd_disque , /* disque ou partition de disque */
	fd_dossier , /* dossier de fichiers */
	fd_fichier , /* fichier dans un dossier */
} ;
...
struct fd
{
	EFI_HANDLE handle ; /* handle UEFI du fichier ou du disque */
 ...
 * @param buf L'adresse où déposer les caractères lus.
 ...
 * @param o Valeur 0 pour ConOut et !=0 pour StdErr.
 * @param msg Le texte UTF-8 à écrire.
 * @param n Le nombre d'octets à écrire.
 * @return Le nombre d'octets effectivement écrits
 *         ou -1 en cas d'erreur (consulter errno).
 */
static ssize_t write_console( o , msg , n )
	EFI_HANDLE o ;
	const char * msg ;
	size_t n ;
{
	...
	for ( i = 0 ; i < n ; ) {
		...
		if ( UEFI_console_putchar( uni , (int) (long) o ) == -1 ) {
			/* output error */
			err = errno ;
			break ;
		}
 ...
 * @param fd Le numéro du descripteur du disque ou fichier où écrire.
