                                  ...
                                  int argcount)
{
    ...
    cc = FFI_DEFAULT_ABI;
#if defined(UEFI_MSVC_32) && !defined(MS_WIN64) && !defined(_WIN32_WCE) && !defined(UEFI_MSVC_64)
            ...
            PyObject *checker)
{
    ...
    void *resbuf = NULL;        // UEFI_C_SOURCE
    struct argument *args, *pa;
    ffi_type **atypes = NULL;   // UEFI_C_SOURCE
    ffi_type *rtype;
    void **avalues = NULL;      // UEFI_C_SOURCE
    PyObject *retval = NULL;    // UEFI_C_SOURCE
...
#ifdef UEFI_C_SOURCE
    {
      int tempcount = argcount;
      if (argcount == 0) tempcount = 1;
      args = (struct argument *)malloc(sizeof(struct argument) * tempcount);
    }
#else
...

    {
      ...
      avalues = (void **)malloc(sizeof(void *) * tempcount);
      atypes = (ffi_type **)malloc(sizeof(ffi_type *) * tempcount);
    }
#else
...
#ifdef _UNICODE
#ifdef UEFI_C_SOURCE
    name = malloc((PyString_Size(nameobj) + 1) * sizeof(WCHAR));
#else
...
#ifdef UEFI_C_SOURCE
        buf = malloc(strlen(PyString_AS_STRING(cls)) + 3 + 1);
#else
...
#ifdef UEFI_C_SOURCE
        buf = malloc(strlen(typ->tp_name) + 3 + 1);
#else
