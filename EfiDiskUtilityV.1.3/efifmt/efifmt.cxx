...
#ifndef EFI64
EFI2_DRIVER_ENTRY_POINT(InitializeEfiFmtApplication)
#endif
    ...
    )
{
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((D_ERROR, (CHAR8*)"InitShellApp: Application not started from Shell\n"));
    ...
    //
    Status = BS->LocateProtocol (&ShellEnvProtocol, NULL, (VOID**)&SE);
    if (EFI_ERROR (Status)) {
        Print (TEXT("%EInitShellApp: Shell environment interfaces not found%N\n"));
        BS->Exit (ImageHandle, Status, 0, NULL);
    }

    DEBUG((D_INFO,(CHAR8*)"EFIFMT application started\n"));

    Print(TEXT("%s Version %d.%d. Based on UEFI Version %s\n"),
        TOOL_NAME,
        TOOL_VERSION_MAJOR,
        TOOL_VERSION_MINOR,
...
//    }
    ...
    // Shouldn't limit the cluster size as long as it is reasonable.
...
//    }
    ...
    //
//
// BugBug- Should we support compression.  If so, what algorithm
//
//    if (compress) {
//
...
//        return 1; BUGBUG should i fail this?
    ...
    intnum = dp_drive->QuerySectors() * dp_drive->QuerySectorSize() % 1048576;

    //
    // Divide so that we achieve a value no greater than 99
    //
    if (bigint < 3) { 
      intnum = intnum / 9680;
    } 
    ...
    if (bigint < 3) {
      message->Display("%d%d", bigint.GetLowPart(), intnum);
    } else {
...
//                DEBUG((D_INFO,(CHAR8*)"FS type forced, user hasn't forced a cluster size, picking %d\n",cluster_size));
    ...
    if (file_system_name == fat_name || file_system_name == fat32_name) {
        ...
        if ( cluster_size==0 ) {
            if (/*dp_drive->IsFloppy()*/ dp_drive->IsRemovable() && force_filesystem) {
              fattypestr = "FAT32";
        ...
        dp_drive->FlushBuffers(&drive_name);
