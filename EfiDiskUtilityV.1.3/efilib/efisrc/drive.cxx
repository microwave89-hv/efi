...
DP_DRIVE::OpenDrive(
    ...
    OUT     PULONG      OptimalTransferBlocks,
    IN OUT  PMESSAGE    Message
    )
/*++
...
--*/
{
    ...
    status = BS->HandleProtocol(handle, &blk_io_guid, (VOID**)BlockIoPtr);

    status = BS->HandleProtocol(handle, &dsk_io_guid, (VOID**)DiskIoPtr);
    ...
    while (DevicePathType(nextDevPath) != END_DEVICE_PATH_TYPE) {
        DEBUG((D_INFO,(CHAR8*)"NextDevPath: %d,%d,%d,%s\n",
                 nextDevPath->Type,
                 nextDevPath->SubType,
                 *(USHORT*)nextDevPath->Length,
                 DevicePathToStr(nextDevPath)));
    if(DevicePathType(nextDevPath) == MEDIA_DEVICE_PATH) 
        break;
    else
    ...
    if  ((DevicePathType(nextDevPath) == MEDIA_DEVICE_PATH)) {
        ...
        //
        CopyMem(&_partition_number, &( ( HARDDRIVE_DEVICE_PATH * ) nextDevPath ) -> PartitionNumber, sizeof ( ( HARDDRIVE_DEVICE_PATH * ) nextDevPath ) -> PartitionNumber);

        DEBUG((D_INFO,(CHAR8*)"Partition Number %d\n", _partition_number));

        if (_partition_number == 0) {
        ...
        // 
        // We are now at the end, so it is the full disk
        //
        DEBUG((D_INFO,(CHAR8*)"There is no partition\n"));
        _partition_number = 0;
        //
        // Only support un-partitioned removeable media for formatting
        // 
        if ((*BlockIoPtr)->Media->RemovableMedia == FALSE) {
              return STATUS_UNSUCCESSFUL;
        }
    }
    ...
    // now we retrieve the interface pointer from the object
    status = BS->HandleProtocol(handle, &blk_io_guid, (VOID**)&_device_block_io);

    status = BS->HandleProtocol(handle, &dsk_io_guid, (VOID**)&_device_disk_io);

    DEBUG((D_INFO,(CHAR8*)"Leaving OpenDrive.\n"));

    *OptimalTransferBlocks = 1;
    if (_device_block_io->Revision >= EFI_BLOCK_IO_PROTOCOL_REVISION2 &&
        _device_block_io->Media->LogicalBlocksPerPhysicalBlock != 0) {
        *OptimalTransferBlocks = _device_block_io->Media->LogicalBlocksPerPhysicalBlock;
    }

    if (_device_block_io->Revision >= EFI_BLOCK_IO_PROTOCOL_REVISION3 &&
        _device_block_io->Media->OptimalTransferLengthGranularity != 0) {
        *OptimalTransferBlocks = Lcm (
            *OptimalTransferBlocks,
            _device_block_io->Media->OptimalTransferLengthGranularity
            );
    }
...
--*/
{
    ...
    partition_info.PartitionType = EFI_PARTITION;  
...
Routine Description:
  ...
  Flush the Block_Io buffers (typically used at end of format)
...
--*/
{
    ...
    // now we retrieve the interface pointer from the object
    status = BS->ReinstallProtocolInterface(handle, &blk_io_guid, (VOID*)BlockIoPtr, (VOID*)BlockIoPtr);
...
--*/
{
    ...
    TestBuffer1 = AllocatePool(MaxSectorsInVerify * QuerySectorSize());
    TestBuffer2 = AllocatePool(MaxSectorsInVerify * QuerySectorSize());
    ...
    SetMem(TestBuffer1, MaxSectorsInVerify * QuerySectorSize(), 0xAA);
    SetMem(TestBuffer2, MaxSectorsInVerify * QuerySectorSize(), 0x55);
