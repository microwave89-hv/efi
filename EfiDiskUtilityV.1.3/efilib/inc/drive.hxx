...
private:
    ...
    ULONG       _optimal_transfer_blocks;
...
--*/
{
  ...
  // 
  // Some person decided that the above test makes sense.  I assert it does not, and am changing it.  MARMAR
  // The damned program thinks this should be the way it has it - uttery messed up - uncommenting the above line
  //
//    return IsRemovable() && _actual.MediaType == RemovableMedia;
}
...
INLINE
ULONG
DP_DRIVE::QueryOptimalTransferLength(
                                    ) CONST
/*++

Routine Description:

    This routine returns the optimal transfer length for the drive.
...
Return Value:
    ...
    The optimal transfer length for the drive.

--*/
{
    return _optimal_transfer_blocks * _actual.SectorSize;
}
