...
/*
    Return the Block Io parameters
*/
{
    EFI_BLOCK_IO    *BlkIo;

    //
    // Set the parameters to the default value.
    //
    *LowestAlignedLba      = 0;
    *OptimalTransferBlocks = 1;
    ...
    if (BlkIo->Revision >= EFI_BLOCK_IO_PROTOCOL_REVISION2) {
        *LowestAlignedLba      = BlkIo->Media->LowestAlignedLba;
        if (BlkIo->Media->LogicalBlocksPerPhysicalBlock != 0) {
            *OptimalTransferBlocks = BlkIo->Media->LogicalBlocksPerPhysicalBlock;
        }
    }
